...
#header-img: img/blog/19.jpg    #这篇文章标题背景图片
catalog: true                       # 是否归档
tags:                               #标签
    - boltdb
    - database
...
任何页都有可能作为FREELIST PAGE使用, 在META PAGE中的`freelist_pgid`记录了它当前的位置;
...
B+树算法本身就不做过多的赘述了; BoltDB将B+树每个节点, 都持久化到一段连续的页中;
...
B+树节点中的数字为其占用的页号, 如根节点的4表示根节点的内容存储在4页开始的一些页中;![](pics/bolt0.png)

#### 1. 修改/添加

某个事务调用修改/添加操作, 修改了某个叶节点的内容;

BoltDB将改叶节点的内容copy一份出来, 缓存在内存中, 如下图红色部分;![](pics/bolt1.png)

#### 2. 删除

该事务调用了删除操作, 于是影响了某个叶节点内的内容;

同理, 拷贝出一份维护在内存中;![](pics/bolt2.png)
...
假设下面的这个节点进行了分裂, 则会形成如下图:![](pics/bolt3.png)

#### 4. Rebalance-Merge

B+树的另一个性质是如果某个节点内容过少, 会和兄弟节点进行合并, 这也会影响到父节点;

假设下面这个节点进行了合并, 则形成如下图:![](pics/bolt4.png)
...
#### 6. 为B+树申请分页![](pics/bolt5.png)![](pics/bolt6.png)

因为中间两个节点的页号发生了改变, 所以其父亲节点的索引页需要更新, 于是他们的父亲节点也需要改动;

![](pics/bolt7.png)![](pics/bolt8.png)

#### 7. 为freelist申请分页

回收老freelist所使用的页, 为新freelist申请分页:![](pics/bolt10.png)

#### 8. 更新META\_PAGE

由于根节点和freelist的页号发生了改变, META PAGE也会被改变, 改变先缓存在内存中;![](pics/bolt11.png)

#### 9. 持久化B+树节点

接下来真正的持久化B+树节点;

#### ![](pics/bolt12.png)![](pics/bolt13.png)10. 持久化freelist![](pics/bolt14.png)

#### 11. 更新META PAGE到磁盘![](pics/bolt15.png)12. 最后结果![](pics/bolt16.png)
